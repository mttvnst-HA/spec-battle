...
- Default model `claude-sonnet-4-6`; override with `TUNE_MODEL=claude-opus-4-6`. Default timeout 240s per call; override with `TUNE_TIMEOUT_MS=180000`. Transport retries once on `err.code === "ETIMEDOUT"` (not on nonzero exit — those are usually prompt or schema errors — and not on signal-only SIGTERMs, which are broader kernel-kills). On a second timeout, the error message gets a ` (after 1 retry)` suffix and surfaces in `tuning-summary.md`'s `## Last transport error` section (Phase 2.2f).
